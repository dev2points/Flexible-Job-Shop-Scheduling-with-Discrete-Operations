...
def build_constraints(solver, SCO, SFO, feasible_time, s, x, m, xm, y, top_id, args):
    ...
    for op in all_ops:
        ...
        cnf = CardEnc.equals(lits=[m[(key, machine)] for machine in req_machines], bound=1, encoding=EncType.pairwise)
        ...
        for k in range(len(scos) - 1):
            ...
            for t_u in range(est_u, lst_u + 1):
                for machine_u, p_u in op_u['request_list'].items():
                    ...
                    if finish_u > lst_v:
                        solver.add_clause([-x[(key_u, t_u)], -m[(key_u, machine_u)]])
                    elif finish_u > est_v:
                        solver.add_clause([-x[(key_u, t_u)], -m[(key_u, machine_u)], x[(key_v, finish_u)]])
